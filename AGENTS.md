...
- 创建或修改 PR 时，正文必须包含且原样使用二级标题 `## 实现假设清单` 和 `## 代码逻辑图`；不得改名、降级为普通文本或以近义标题替代，即使改动很小也不得省略。
- `## 实现假设清单` 与 `## 代码逻辑图` 两节均不得为空或使用占位内容。前者必须列出实现所依赖的假设（无额外假设时明确写明），后者必须提供反映本次改动实际流程的 Mermaid 代码逻辑图。
- 每次创建或修改 PR 前，必须检查正文中上述两个标题各恰好出现一次，且对应内容完整。
