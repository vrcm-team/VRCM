...
class SettingsDao(private val settingsSettings: Settings) {
    var settings: SettingsData
        ...
        set(value) {
            value.isDarkTheme?.let { settingsSettings.putBoolean(DaoKeys.Settings.IS_DARK_THEME_KEY, it) }
                ?: settingsSettings.remove(DaoKeys.Settings.IS_DARK_THEME_KEY)
            value.themeColor?.let { settingsSettings.putString(DaoKeys.Settings.THEME_COLOR_KEY, it) }
            value.languageTag?.let { settingsSettings.putString(DaoKeys.Settings.LANGUAGE_TAG_KEY, it) }
    ...
    var rememberVersion: String?
        ...
        set(value) = if (!value.isNullOrEmpty()) settingsSettings.putString(DaoKeys.Settings.REMEMBER_VERSION_KEY, value)
        else settingsSettings.remove(DaoKeys.Settings.REMEMBER_VERSION_KEY)
