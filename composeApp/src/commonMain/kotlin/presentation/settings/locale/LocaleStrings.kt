package io.github.vrcmteam.vrcm.presentation.settings.locale

import androidx.compose.runtime.Composable
import androidx.compose.runtime.Stable
import io.github.vrcmteam.vrcm.presentation.settings.LocalSettingsState

/**
 * Base strings shared by every locale.
 *
 * Strings are properties rather than constructor parameters so this catalog is not constrained by
 * the JVM's 255-parameter method limit. Locale objects override every translated property.
 */
@Stable
sealed class LocaleStrings {
    open val startupDialogTitle: String = "New version available"
    open val startupDialogUpdate: String = "Update"
    open val startupDialogIgnore: String = "Ignore"
    open val startupDialogRememberVersion: String = "Ignore This Version Update"
    open val apiRequestRateLimited: String = "Too many requests. Please try again later."
    open val authLoginTitle: String = "Login"
    open val authLoginButton: String = "LOGIN"
    open val authLoginUsername: String = "Username"
    open val authLoginPassword: String = "Password"
    open val authVerifyTitle: String = "Verify"
    open val authVerifyButton: String = "VERIFY"
    open val authCurrent: String = "Current"
    open val desktopWindowMinimize: String = "Minimize"
    open val desktopWindowMaximize: String = "Maximize"
    open val desktopWindowRestore: String = "Restore"
    open val desktopWindowClose: String = "Close"
    open val widePaneEmptyTitle: String = "There's not even a mirror here yet"
    open val widePaneEmptyHint: String = "Pick a friend, world, or group on the left to take a look"
    open val fiendLocationPagerWebsite: String = "Active on the Website"
    open val fiendLocationPagerPrivate: String = "Friends in Private Worlds"
    open val fiendLocationPagerTraveling: String = "Friends is Traveling"
    open val fiendLocationPagerLocation: String = "by Location"
    open val currentUserLocation: String = "You are here"
    open val fiendListPagerSearch: String = "Search"
    open val users: String = "Users"
    open val worlds: String = "Worlds"
    open val groups: String = "Groups"
    open val publicSearchPrompt: String = "Enter a name to search"
    open val publicSearchNoResults: String = "No matching results"
    open val publicSearchFailed: String = "Search failed. Try again."
    open val publicSearchRefreshFailed: String = "Search failed. Previous results are still shown."
    open val notificationFriendRequest: String = "wants to be your friend"
    open val homeNotificationEmpty: String = "No Notification Yet"
    open val homeStatusEdit: String = "Edit Status"
    open val homeUpdateStatus: String = "Update Status"
    open val stettingLanguage: String = "Language"
    open val stettingThemeMode: String = "ThemeMode"
    open val stettingSystemThemeMode: String = "System"
    open val stettingLightThemeMode: String = "Light"
    open val stettingDarkThemeMode: String = "Dark"
    open val stettingThemeColor: String = "ThemeColor"
    open val stettingClipboardReading: String = "Read clipboard"
    open val stettingLogout: String = "Logout"
    open val stettingAbout: String = "About Application"
    open val stettingVersion: String = "Version"
    open val stettingClearCache: String = "Clear Cache"
    open val stettingAlreadyLatest: String = "Already Latest"
    open val stettingFriendActivity: String = "Friend activity"
    open val stettingFriendPresenceNotifications: String = "Presence alerts"
    open val stettingBoopNotifications: String = "Boop alerts"
    open val stettingBackgroundMonitoring: String = "Background monitoring"
    open val stettingBackgroundSettings: String = "Background settings"
    open val stettingBackgroundNotifications: String = "Notifications"
    open val stettingBackgroundBattery: String = "Battery"
    open val stettingNotificationSettings: String = "Notification settings"
    open val stettingAppManagement: String = "App settings"
    open val stettingBatterySettings: String = "Battery settings"
    open val stettingStatusEnabled: String = "Enabled"
    open val stettingStatusDisabled: String = "Disabled"
    open val stettingStatusAllowed: String = "Allowed"
    open val stettingStatusManaged: String = "System managed"
    open val stettingBackgroundPermissionRequired = "Notification permission is required"
    open val stettingBackgroundUnavailable = "Background monitoring is unavailable"
    open val rewardCodeEntry: String = "Redeem reward code"
    open val rewardCodeTitle: String = "Reward code"
    open val rewardCodeInputLabel: String = "Code"
    open val rewardCodeSubmit: String = "Redeem"
    open val rewardCodeSubmitting: String = "Redeeming"
    open val rewardCodeRequired: String = "Enter a code"
    open val rewardCodeFailed: String = "Code could not be redeemed"
    open val rewardCodeSessionUnavailable: String = "The current login session is unavailable"
    open val rewardCodeSuccess: String = "Reward redeemed"
    open val rewardCodeBadge: String = "Badge"
    open val rewardCodeItem: String = "Item"
    open val rewardCodeReward: String = "Reward"
    open val inviteMessageSlotsTitle: String = "Invite messages"
    open val inviteMessageSettingsSummary: String = "Manage"
    open val inviteMessageTypeMessage: String = "Invites"
    open val inviteMessageTypeResponse: String = "Invite replies"
    open val inviteMessageTypeRequest: String = "Invite requests"
    open val inviteMessageTypeRequestResponse: String = "Request replies"
    open val inviteMessageSessionUnavailable: String = "This account is no longer signed in."
    open val inviteMessageLoadFailed: String = "Invite messages could not be loaded."
    open val inviteMessageEmpty: String = "No message slots were found."
    open val inviteMessageSlotLabel: String = "Slot %d"
    open val inviteMessageCooldownRemaining: String = "%d min remaining"
    open val inviteMessageUnavailable: String = "Unavailable"
    open val inviteMessageEdit: String = "Edit message"
    open val inviteMessageReset: String = "Reset message"
    open val inviteMessageEditTitle: String = "Edit slot %d"
    open val inviteMessageFieldLabel: String = "Message"
    open val inviteMessageRequired: String = "Message cannot be empty."
    open val inviteMessageTooLong: String = "Keep the message within 64 characters."
    open val inviteMessageUnchanged: String = "No changes"
    open val inviteMessageResetTitle: String = "Reset message"
    open val inviteMessageResetMessage: String = "Reset slot %d to its default message?"
    open val inviteMessageResetConfirm: String = "Reset"
    open val inviteMessageUpdated: String = "Message updated"
    open val inviteMessageResetDone: String = "Message reset"
    open val inviteMessageCooldownError: String = "This slot is still cooling down. Try again later."
    open val inviteMessageUpdateFailed: String = "Message could not be updated."
    open val inviteMessageResetFailed: String = "Message could not be reset."
    open val playerModerationTitle: String = "Player management"
    open val playerModerationEntryDescription: String = "View and manage records"
    open val playerModerationRefresh: String = "Refresh player management records"
    open val playerModerationSignedOut: String = "Sign in to view player management records"
    open val playerModerationLoadFailed: String = "Player management records could not be loaded"
    open val playerModerationEmpty: String = "No player management records"
    open val playerModerationFilterAll: String = "All"
    open val playerModerationUnknownPlayer: String = "Unknown player"
    open val playerModerationTypeMute: String = "Mute voice"
    open val playerModerationTypeUnmute: String = "Unmute voice"
    open val playerModerationTypeBlock: String = "Block"
    open val playerModerationTypeUnblock: String = "Unblock"
    open val playerModerationTypeInteractOff: String = "Disable interactions"
    open val playerModerationTypeInteractOn: String = "Enable interactions"
    open val playerModerationTypeMuteChat: String = "Mute chatbox"
    open val playerModerationTypeUnmuteChat: String = "Unmute chatbox"
    open val playerModerationTypeUnknown: String = "Unknown type: %s"
    open val playerModerationPlayerActions: String = "Player actions"
    open val playerModerationManagePlayer: String = "Manage player"
    open val playerModerationRemoveSetting: String = "Clear player setting"
    open val playerModerationRemoveConfirmTitle: String = "Clear this player setting?"
    open val playerModerationRemoveConfirmMessage: String =
        "Clear \"%type%\" for %player%? This cannot be undone."
    open val notificationSectionFriendPresence: String = "Friend presence"
    open val notificationOnlineDescription: String = "Alerts when a friend enters the game"
    open val notificationFriendOffline: String = "Offline alerts"
    open val notificationOfflineDescription: String = "Alerts when a friend leaves the game"
    open val notificationSectionFilter: String = "Who to alert about"
    open val notificationFilterDescription: String =
        "Applies to both online and offline alerts. A choice made for one friend wins over their groups."
    open val notificationFilterBlacklist: String = "Exclude selected"
    open val notificationFilterWhitelist: String = "Only selected"
    open val notificationFilterGroups: String = "Favorite groups"
    open val notificationFilterFriends: String = "Individual friends"
    open val notificationFilterFriendsDescription: String =
        "Leave a friend on Follow group to use the group rule above."
    open val notificationOverrideFollowGroup: String = "Follow group"
    open val notificationOverrideAlways: String = "Always"
    open val notificationOverrideNever: String = "Never"
    open val notificationOverrideFollowGroupShort: String = "Group"
    open val notificationOverrideAlwaysShort: String = "Alert"
    open val notificationOverrideNeverShort: String = "Mute"
    open val notificationSectionInbox: String = "Inbox"
    open val notificationBack: String = "Back"
    open val notificationRefresh: String = "Refresh notifications"
    open val notificationRefreshFailed: String = "Notifications could not be refreshed. Existing items are still available."
    open val notificationMarkRead: String = "Mark as read"
    open val notificationDelete: String = "Delete notification"
    open val notificationReplyBoop: String = "Reply to Boop"
    open val notificationReply: String = "Reply"
    open val notificationDecline: String = "Decline"
    open val notificationUnsubscribe: String = "Unsubscribe"
    open val notificationReplyWithPhoto: String = "Reply with Gallery photo"
    open val notificationRetryPhotoResponse: String = "Retry photo response"
    open val notificationPhotoPreparing: String = "Preparing Gallery photo..."
    open val notificationPhotoResponding: String = "Sending photo response..."
    open val notificationPhotoResponseSuccess: String = "Photo response sent"
    open val notificationPhotoPreparationFailed: String = "Gallery photo could not be prepared. Try again."
    open val notificationAccept: String = "Accept"
    open val notificationIgnore: String = "Ignore"
    open val notificationBlock: String = "Block"
    open val notificationExternalLinkTitle: String = "Open external link?"
    open val notificationExternalLinkMessage: String = "This notification will open %s in your browser."
    open val notificationExternalLinkFailed: String = "The external link could not be opened."
    open val notificationUnsupportedLink: String = "This notification link is unsupported or unsafe."
    open val notificationExpand: String = "Show details"
    open val notificationCollapse: String = "Hide details"
    open val notificationBoopDescription: String = "Alerts for every Boop you receive"
    open val notificationFriendRequestAlert: String = "Friend requests"
    open val notificationFriendRequestAlertDescription: String = "Alerts for every incoming friend request"
    open val notificationGroupInvite: String = "Group invitation"
    open val notificationGroupAnnouncement: String = "Group notifications"
    open val notificationGroupAnnouncementDescription: String =
        "Alerts for announcements, events, and management messages from groups you joined"
    open val notificationSectionServiceStatus: String = "Service status"
    open val notificationVrchatStatus: String = "VRChat service alerts"
    open val notificationVrchatStatusDescription: String =
        "Alerts when VRChat reports an outage and when services recover"
    open val notificationSectionBackground: String = "Background"
    open val notificationBackgroundDescription: String =
        "Keeps monitoring friend activity while the app is in the background"
    open val profileFriendRequestSent: String = "Friend Request Sent"
    open val profileSendFriendRequest: String = "Send Friend Request"
    open val profileFriendRequestDeleted: String = "Friend Request Deleted"
    open val profileDeleteFriendRequest: String = "Delete Friend Request"
    open val profileAcceptFriendRequest: String = "Accept Friend Request"
    open val profileFriendRequestAccepted: String = "Friend Request Accepted"
    open val profileUnfriended: String = "Friend is Unfriended"
    open val profileUnfriend: String = "Unfriend"
    open val profileReportUser: String = "Report suspected hacking/avatar theft"
    open val profileReportTitle: String = "Report user"
    open val profileReportMessage: String =
        "Report %name% for suspected hacking or avatar theft? Only submit this report if you observed the behavior."
    open val profileReportSubmit: String = "Submit report"
    open val profileReportSuccess: String = "Report submitted"
    open val profileReportFailed: String = "Failed to submit report. Please try again."
    open val profileViewJsonData: String = "View JSON data"
    open val profileViewGallery: String = "View Gallery"
    open val profileViewMutualFriends: String = "View Mutual Friends"
    open val profileViewFriendNetwork: String = "View Friend Network"
    open val profileEditProfile: String = "Edit Profile"
    open val profileChatboxModerationMute: String = "Mute Chatbox messages"
    open val profileChatboxModerationUnmute: String = "Unmute Chatbox messages"
    open val profileChatboxModerationChecking: String = "Checking Chatbox setting..."
    open val profileChatboxModerationRetry: String = "Retry Chatbox setting"
    open val profileChatboxModerationMuting: String = "Muting Chatbox messages..."
    open val profileChatboxModerationUnmuting: String = "Unmuting Chatbox messages..."
    open val profileChatboxModerationMuted: String = "Chatbox messages muted"
    open val profileChatboxModerationUnmuted: String = "Chatbox messages unmuted"
    open val profileChatboxModerationUpdateFailed: String = "Could not update Chatbox setting"
    open val profileVoiceModerationMute: String = "Mute player voice"
    open val profileVoiceModerationUnmute: String = "Unmute player voice"
    open val profileVoiceModerationChecking: String = "Checking voice setting..."
    open val profileVoiceModerationRetry: String = "Retry voice setting"
    open val profileVoiceModerationMuting: String = "Muting player voice..."
    open val profileVoiceModerationUnmuting: String = "Unmuting player voice..."
    open val profileVoiceModerationMuted: String = "Player voice muted"
    open val profileVoiceModerationUnmuted: String = "Player voice unmuted"
    open val profileVoiceModerationUpdateFailed: String = "Could not update voice setting"
    open val profileInteractionChecking: String = "Checking interaction setting..."
    open val profileInteractionClose: String = "Disable Player Interactions"
    open val profileInteractionRestore: String = "Restore Player Interactions"
    open val profileInteractionClosing: String = "Disabling interactions..."
    open val profileInteractionRestoring: String = "Restoring interactions..."
    open val profileInteractionRetry: String = "Couldn't load interaction setting. Retry"
    open val profileInteractionUnavailable: String = "Interaction setting unavailable"
    open val profileInteractionCloseConfirmTitle: String = "Disable Player Interactions"
    open val profileInteractionCloseConfirmMessage: String = "Disable interactions with %s?"
    open val profileInteractionRestoreConfirmTitle: String = "Restore Player Interactions"
    open val profileInteractionRestoreConfirmMessage: String = "Explicitly allow interactions with %s?"
    open val profileInteractionClosedSuccess: String = "Player interactions disabled"
    open val profileInteractionRestoredSuccess: String = "Player interactions restored"
    open val profileInteractionLoadFailed: String = "Couldn't load the interaction setting"
    open val profileInteractionUpdateFailed: String = "Couldn't update the interaction setting"
    open val editProfileTitle: String = "Edit Profile"
    open val editProfileStatus: String = "Social Status"
    open val editProfileStatusDescription: String = "Status description"
    open val editProfileStatusType: String = "Status type"
    open val editProfilePronouns: String = "Pronouns"
    open val editProfileBio: String = "Bio"
    open val editProfileAvatarCopying: String = "Allow Avatar Copying"
    open val editProfileAvatarCopyingDescription: String =
        "Lets other users copy the public avatar you are currently using."
    open val editProfileAvatarCopyingLoadFailed: String =
        "Could not load this privacy setting."
    open val editProfileAvatarCopyingUpdateFailed: String =
        "Could not update this privacy setting. Your previous choice is unchanged."
    open val editProfileAvatarCopyingEnabled: String = "Avatar copying enabled"
    open val editProfileAvatarCopyingDisabled: String = "Avatar copying disabled"
    open val editProfileSocialLinks: String = "Social Links"
    open val editProfileBoopPrivacy: String = "Allow Boops"
    open val editProfileSocialLink: String = "Link %s"
    open val editProfileAddSocialLink: String = "Add Link"
    open val editProfileRemoveSocialLink: String = "Remove link"
    open val editProfileSocialLinksHint: String = "VRChat validates links when you save. Failed changes remain here for editing and retry."
    open val editProfileLanguage: String = "Language"
    open val editProfileAddLanguage: String = "Add Language"
    open val editProfileLanguageHint: String = "Max 3 languages. Tap to remove."
    open val editProfileSave: String = "Save"
    open val editProfileStatusOnline: String = "Online"
    open val editProfileStatusJoinMe: String = "Join Me"
    open val editProfileStatusAskMe: String = "Ask Me"
    open val editProfileStatusBusy: String = "Do Not Disturb"
    open val editProfileUpdateSuccess: String = "Profile updated"
    open val editProfileUpdateFailed: String = "Could not update the privacy setting."
    open val mutualFriendsTitle: String = "Mutual Friends"
    open val mutualFriendsEmpty: String = "No mutual friends with %s"
    open val mutualFriendsLoadFailed: String = "Failed to load mutual friends"
    open val mutualFriendsCount: String = "Mutual Friends: %total%"
    open val mutualFriendsCountWithHidden: String = "Mutual Friends: %total% (Hidden %hidden%)"
    open val mutualFriendsHiddenUser: String = "Hidden Mutual"
    open val friendNetworkTitle: String = "Friend Network"
    open val friendNetworkLastUpdated: String = "Last updated: %s"
    open val friendNetworkCacheHint: String = "Cached data. Tap refresh to update."
    open val friendNetworkBuilding: String = "Building network... %s"
    open val friendNetworkEmpty: String = "No network data yet"
    open val friendNetworkViewCommunity: String = "Circles"
    open val friendNetworkViewEgo: String = "Around me"
    open val locationDialogOwner: String = "Owner"
    open val locationDialogAuthor: String = "Author"
    open val locationDialogDescription: String = "Description"
    open val locationDialogTags: String = "Tags"
    open val locationInvited: String = "Invited"
    open val locationInviteMe: String = "Invite Me"
    open val createInstance: String = "Create Instance"
    open val accessType: String = "Access Type"
    open val regionType: String = "Region"
    open val confirm: String = "Confirm"
    open val cancel: String = "Cancel"
    open val favoriteWorld: String = "Favorite World"
    open val favoriteAvatar: String = "Favorite Avatar"
    open val editFavorite: String = "Edit Favorite"
    open val favoriteAddSuccess: String = "Add Favorite Success"
    open val favoriteAddFailed: String = "Add Favorite Failed"
    open val favoriteAlreadyExists: String = "it is already in your favorites"
    open val favoriteGroupSelect: String = "Select favorite group"
    open val favoriteSelectGroup: String = "Select Group"
    open val favoriteDefaultGroup: String = "Default Group"
    open val favoriteRemoveSuccess: String = "Remove Favorite Success"
    open val favoriteRemoveFailed: String = "Remove Favorite Failed"
    open val favoriteMoveFailed: String = "Move Favorite Failed"
    open val favoriteMoveSuccess: String = "Move Favorite Success"
    open val favoriteMoveToAnotherGroup: String = "Move Favorite"
    open val selectFavoriteGroup: String = "Select Favorite Group"
    open val favoriteGroupEditAction: String = "Edit favorite group"
    open val favoriteGroupEditTitle: String = "Edit favorite group"
    open val favoriteGroupEditName: String = "Display name"
    open val favoriteGroupEditVisibility: String = "Visibility"
    open val favoriteGroupVisibilityPrivate: String = "Private"
    open val favoriteGroupVisibilityFriends: String = "Friends"
    open val favoriteGroupVisibilityPublic: String = "Public"
    open val favoriteGroupEditSave: String = "Save"
    open val favoriteGroupEditSaving: String = "Saving..."
    open val favoriteGroupEditNameRequired: String = "Display name cannot be empty."
    open val favoriteGroupEditFailed: String = "Failed to update favorite group."
    open val favoriteGroupEditSuccess: String = "Favorite group updated."
    open val remove: String = "Remove"
    open val instanceCreateSuccess: String = "Successfully created instance and sent invite"
    open val instanceCreateSuccessButInviteFailed: String = "Instance created successfully but invite failed"
    open val instanceCreateFailed: String = "Failed to create instance"
    open val worldPublishAction: String = "Publish to Community Labs"
    open val worldUnpublishAction: String = "Unpublish World"
    open val worldPublishConfirmationTitle: String = "Publish World?"
    open val worldUnpublishConfirmationTitle: String = "Unpublish World?"
    open val worldPublishConfirmationMessage: String =
        "Publish %s to Community Labs? Publishing is limited to one world per week."
    open val worldUnpublishConfirmationMessage: String = "Unpublish %s and make it private?"
    open val worldPublishSuccess: String = "World published"
    open val worldUnpublishSuccess: String = "World unpublished"
    open val worldPublishFailed: String = "Failed to publish world"
    open val worldUnpublishFailed: String = "Failed to unpublish world"
    open val worldPublishUnavailable: String =
        "This world is not currently eligible for publishing. You can publish at most one world per week."
    open val worldPublishAvailabilityCheckFailed: String =
        "Could not check publishing availability. Refresh to try again."
    open val worldPublicationRefreshRequired: String =
        "The request completed, but the current status could not be confirmed. Refresh before trying again."
    open val worldPublicationCacheSyncFailed: String =
        "World status updated, but the cached data could not be saved. Refresh before trying again."
    open val instanceCloseAction: String = "Close instance"
    open val instanceCloseCheckingPermission: String = "Checking permission"
    open val instanceCloseConfirmTitle: String = "Close this instance?"
    open val instanceCloseConfirmMessage: String = "This instance will immediately stop accepting new users. This action cannot be undone."
    open val instanceCloseInProgress: String = "Closing instance"
    open val instanceCloseSuccess: String = "Instance closed"
    open val instanceCloseFailed: String = "Failed to close instance"
    open val instanceClosePermissionDenied: String = "You do not have permission to close this instance"
    open val instanceCloseSessionChanged: String = "The account session changed. Open the instance and try again."

    // World Search
    open val worldSearchAdvancedOptions: String = "Advanced Search Options"
    open val worldSearchFeaturedOnly: String = "Featured Worlds Only"
    open val worldSearchSortBy: String = "Sort By"
    open val worldSearchOrder: String = "Order"
    open val worldSearchDescending: String = "Descending"
    open val worldSearchAscending: String = "Ascending"
    open val worldSearchResultCount: String = "Results Count"
    open val worldSearchResultsFormat: String = "%d results"

    // Sort Options
    open val worldSearchSortPopularity: String = "Popularity"
    open val worldSearchSortHeat: String = "Heat"
    open val worldSearchSortTrust: String = "Trust"
    open val worldSearchSortShuffle: String = "Shuffle"
    open val worldSearchSortRandom: String = "Random"
    open val worldSearchSortFavorites: String = "Favorites"
    open val worldSearchSortCreated: String = "Created"
    open val worldSearchSortUpdated: String = "Updated"
    open val worldSearchSortRelevance: String = "Relevance"
    open val worldSearchSortName: String = "Name"

    // FriendListPager
    open val friendListPagerAllFriends: String = "All Friends"
    open val friendListPagerAllWorlds: String = "All Worlds"
    open val friendDirectoryTitle: String = "Friends"
    open val friendDirectoryInGame: String = "In VRChat"
    open val friendDirectoryWeb: String = "Online on Web"
    open val friendDirectoryPrivate: String = "Private or unavailable"
    open val friendDirectoryOffline: String = "Offline"
    open val friendDirectoryTraveling: String = "Traveling"
    open val friendDirectoryPublicWorld: String = "Public world"
    open val friendDirectoryFriendsWorld: String = "Friends-only world"
    open val friendDirectoryInviteWorld: String = "Invite-only world"
    open val friendDirectoryLastActive: String = "Last active"
    open val friendDirectoryEmpty: String = "No friends yet"
    open val friendDirectoryNoMatches: String = "No friends match this filter"
    open val friendDirectoryLoadFailed: String = "Couldn't refresh friends"
    open val friendDirectorySelect: String = "Select friends to remove"
    open val friendDirectorySelectedCount: String = "%d selected"
    open val friendDirectorySelectAll: String = "Select all shown"
    open val friendDirectoryClearSelection: String = "Clear shown"
    open val friendDirectoryRemoveSelected: String = "Remove selected friends"
    open val friendDirectoryRemoveConfirmTitle: String = "Remove friends?"
    open val friendDirectoryRemoveConfirmMessage: String =
        "This will remove %d selected friends. This action cannot be undone."
    open val friendDirectoryRemovingProgress: String = "Removing %d of %d"
    open val friendDirectoryRemoveSuccess: String = "Removed %d friends"
    open val friendDirectoryRemovePartialFailure: String = "Removed %d friends; %d failed"
    open val friendDirectoryRemoveFailed: String = "Failed to remove %d friends"

    // WorldProfileScreen
    open val worldProfileDescription: String = "World Description"
    open val worldProfileRooms: String = "Instances"
    open val worldProfileActiveRooms: String = "Active Instances"
    open val worldProfileInformation: String = "World Information"
    open val worldProfileCapacityValue: String = "%count% people"
    open val worldProfileCommunityLabs: String = "Community Labs"
    open val worldProfileMoreActions: String = "More actions"
    open val worldProfileAuthorTags: String = "Author Tags"
    open val worldProfileCreateFirstInstance: String = "Create the first instance"
    open val worldProfileCreateFirstInstanceHint: String = "Select this stand to get started"
    open val worldProfileCapacity: String = "Capacity"
    open val worldProfileOnlineUsers: String = "Total Online Users"
    open val worldProfileVisits: String = "Visits"
    open val worldProfileFavorites: String = "Favorites"
    open val worldProfileHeat: String = "Heat"
    open val worldProfilePopularity: String = "Popularity"
    open val worldProfileVersion: String = "Version"
    open val worldProfileCreatedDate: String = "Created Date"
    open val worldProfilePublishDate: String = "Publish Date"
    open val worldProfileUpdateDate: String = "Update Date"
    open val worldProfileLabReleaseDate: String = "Lab Release Date"
    open val worldPersistenceTitle: String = "Saved world data"
    open val worldPersistenceInitial: String =
        "Check whether this world has saved data for your account."
    open val worldPersistenceCheck: String = "Check"
    open val worldPersistenceChecking: String = "Checking saved data..."
    open val worldPersistenceExists: String = "This world has saved data for your account."
    open val worldPersistenceMissing: String = "No saved data was found for your account in this world."
    open val worldPersistenceDeleted: String = "The saved data for this world has been deleted."
    open val worldPersistenceCheckFailed: String = "Couldn't check saved data. Try again."
    open val worldPersistenceCheckAgain: String = "Check again"
    open val worldPersistenceDelete: String = "Delete data"
    open val worldPersistenceDeleting: String = "Deleting saved data..."
    open val worldPersistenceDeleteFailed: String = "Couldn't delete saved data. Try again."
    open val worldPersistenceDeleteConfirmTitle: String = "Delete saved world data?"
    open val worldPersistenceDeleteConfirmMessage: String =
        "This permanently deletes your account's saved data for this world. This action cannot be undone."
    open val worldDeleteAction: String = "Delete world"
    open val worldDeleteConfirmationTitle: String = "Delete this world?"
    open val worldDeleteConfirmationMessage: String =
        "Delete \"%name%\"? The world will become hidden, and associated files will be " +
            "permanently deleted and cannot be recovered. The world ID will remain permanently reserved."
    open val worldDeleteSuccess: String = "World deleted"
    open val worldDeleteSuccessCacheCleanupFailed: String =
        "World deleted, but its local cached details couldn't be removed."
    open val worldDeleteFailed: String = "Couldn't delete this world. Try again."
    open val worldProfileSetHomeWorld: String = "Set as Home World"
    open val worldProfileResetHomeWorld: String = "Reset Home World"
    open val worldProfileHomeWorldUnavailable: String = "Home World action unavailable"
    open val worldProfileSetHomeWorldConfirmation: String = "Use this world as your Home World?"
    open val worldProfileResetHomeWorldConfirmation: String =
        "Remove your custom Home World and restore VRChat's default?"
    open val worldProfileHomeWorldSetSuccess: String = "Home World updated"
    open val worldProfileHomeWorldResetSuccess: String = "Home World reset"
    open val worldProfileHomeWorldUpdateFailed: String = "Couldn't update Home World"
    open val worldEditTitle: String = "Edit world"
    open val worldEditName: String = "Name"
    open val worldEditDescription: String = "Description"
    open val worldEditCapacity: String = "Capacity"
    open val worldEditRecommendedCapacity: String = "Recommended capacity"
    open val worldEditCapacityHint: String =
        "Capacity must be between 0 and 40; recommended capacity cannot exceed it."
    open val worldEditTags: String = "Tags"
    open val worldEditTagsHint: String = "Enter one complete VRChat tag per line."
    open val worldEditAllowedDomains: String = "Allowed Domains"
    open val worldEditAllowedDomainsHint: String =
        "Enter one domain accepted by VRChat per line."
    open val worldEditSave: String = "Save world details"
    open val worldEditSaving: String = "Saving..."
    open val worldEditInvalidName: String = "World name cannot be empty."
    open val worldEditInvalidCapacity: String = "World capacity must be a whole number from 0 to 40."
    open val worldEditInvalidRecommendedCapacity: String =
        "Recommended capacity must be a whole number and cannot exceed capacity."
    open val worldEditNoChanges: String = "World details have not changed."
    open val worldEditSaved: String = "World details updated."
    open val worldEditSaveFailed: String = "Failed to update world details."
    open val worldImageEditTitle: String = "Manage World Image"
    open val worldImageEditPreview: String = "Preview image"
    open val worldImageEditThumbnail: String = "Cover thumbnail"
    open val worldImageEditHint: String =
        "Choose a JPEG, PNG, or WebP up to 10 MiB. The editor exports a 1920 x 1080 image; VRChat generates the cover thumbnail."
    open val worldImageEditChoose: String = "Choose image"
    open val worldImageEditUpload: String = "Update world image"
    open val worldImageEditUploading: String = "Uploading world image..."
    open val worldImageEditSaved: String = "World preview and cover updated"
    open val worldImageEditUploadFailed: String = "World image upload failed"
    open val worldImageEditAssignmentFailed: String =
        "The image was uploaded, but VRChat could not assign it to the world. Try updating the image again."
    open val worldImageEditRefreshFailed: String =
        "The image was updated, but the latest world details could not be loaded. Refresh the world to verify it."
    open val worldImageEditSessionChanged: String =
        "Your account session changed. Reopen image management and try again."
    open val worldImageEditUnsupportedFormat: String =
        "The image format is unsupported or does not match its extension. Choose a valid JPEG, PNG, or WebP file."
    open val worldImageEditFileTooLarge: String = "The selected image is larger than 10 MiB"
    open val worldImageEditReadFailed: String = "The selected world image could not be read"
    open val unknown: String = "Unknown"

    // CreateInstanceDialog
    open val createInstanceStandardAccessType: String = "Standard Access Type"
    open val createInstanceEnableQueue: String = "Enable Queue Function"
    open val createInstanceGroupAccessType: String = "Group Access Type"
    open val createInstanceGroup: String = "Group"
    open val createInstanceGroupsLoading: String = "Loading available groups..."
    open val createInstanceGroupsFailed: String = "Unable to load available groups"
    open val createInstanceNoEligibleGroups: String = "No groups allow you to create instances"
    open val createInstanceGroupMembers: String = "Group Members"
    open val createInstanceGroupPlus: String = "Group Plus"
    open val createInstanceGroupPublic: String = "Group Public"
    open val createInstanceRoleRestriction: String = "Allowed Roles"
    open val createInstanceAgeGate: String = "Age Gate"
    open val createInstanceDisplayName: String = "Custom Name"
    open val createInstanceOptional: String = "Optional"
    open val createInstanceMinimumPerformance: String = "Minimum Avatar Performance"
    open val createInstancePerformanceDefault: String = "Group Default"
    open val createInstancePerformancePoor: String = "Poor"
    open val createInstancePerformanceMedium: String = "Medium"
    open val createInstancePerformanceGood: String = "Good"
    open val createInstanceGroupRequired: String = "Select an available group"
    open val createInstanceAccessPermissionRequired: String =
        "Your group roles do not allow this instance type"
    open val createInstanceRoleRequired: String = "Select at least one allowed role"
    open val createInstanceInvalidRole: String = "One or more selected roles are no longer available"
    open val createInstanceAgeGatePermissionRequired: String =
        "Your group roles do not allow age-gated instances"
    open val createInstancePerformancePermissionRequired: String =
        "Your group roles do not allow overriding avatar performance"
    open val createInstanceSubmitting: String = "Creating..."

    // GalleryTabPager
    open val galleryTabNoFiles: String = "No %s Files"
    open val galleryTabUploading: String = "Uploading image..."
    open val galleryTabUploadImage: String = "Upload Image"
    open val galleryTabUploadSuccess: String = "Image uploaded successfully"
    open val galleryTabUploadFailed: String = "Image upload failed"
    open val galleryTabLoadFailed: String = "Loading Failed"
    open val galleryTabVrcPlusRequired: String = "Requires VRC+"
    open val galleryTabDelete: String = "Delete"
    open val galleryTabDeleting: String = "Deleting..."
    open val galleryTabDeleteSuccess: String = "Deleted successfully"
    open val galleryTabDeleteFailed: String = "Delete failed"
    open val galleryTabCropPrintBorder: String = "Crop Print border"
    // Print image editor
    open val printEditorTitle: String = "Edit Print"
    open val printEditorBack: String = "Back"
    open val printEditorUpload: String = "Upload"
    open val printEditorRotateLeft: String = "Rotate left"
    open val printEditorRotateRight: String = "Rotate right"
    open val printEditorFlipHorizontal: String = "Flip horizontally"
    open val printEditorFlipVertical: String = "Flip vertically"
    open val printEditorZoom: String = "Zoom"
    open val printEditorFillWhiteBorder: String = "Fill empty area with white"
    open val printEditorShowFullImage: String = "Show full image"
    open val printEditorReset: String = "Reset"
    open val printEditorProcessing: String = "Preparing image..."
    open val printEditorUploading: String = "Uploading print..."
    open val printEditorUnsupportedFormat: String = "Unsupported image format. Use JPEG or PNG; HEIC/HEIF support depends on the system."
    open val printEditorFileTooLarge: String = "The selected file exceeds 50 MiB."
    open val printEditorImageTooLarge: String = "The selected image exceeds 100 megapixels."
    open val printEditorOutputTooLarge: String =
        "The edited image is too large to prepare safely. Try a smaller source image."
    open val printEditorDesktopRegionDecodeUnavailable: String =
        "This image is too large for safe HEIC/HEIF processing on Desktop. Use JPEG or PNG."
    open val printEditorReadFailed: String = "Failed to read the image: %s"
    open val printEditorDecodeFailed: String = "The image could not be decoded."
    open val printEditorRenderFailed: String = "The image could not be prepared for upload."
    open val printEditorUploadAuthenticationFailed: String = "Authentication expired. Sign in again and retry."
    open val printEditorUploadPermissionFailed: String = "VRChat rejected the Print upload. Check that this account has VRC+."
    open val printEditorUploadNetworkFailed: String = "A network error interrupted the upload. Check your connection and retry."
    open val printEditorUploadServerFailed: String = "The VRChat Print service is unavailable. Try again later."
    open val printEditorUploadUnknownFailed: String = "Print upload failed. Try again."
    open val printEditorUploaded: String = "Print uploaded"
    open val printEditorSessionExpired: String = "This editing session has expired. Select the image again."

    // GalleryScreen
    open val galleryScreenTitle: String = "Gallery"
    
    // ImagePreviewDialog
    open val imageSaveError: String = "Image save error: %s"
    open val imageSave: String = "Save image"
    open val imageSaveSuccess: String = "Image saved to gallery"
    open val imageSaveFailed: String = "Failed to save image"
    open val imageShare: String = "Share image"
    open val imageShareFailed: String = "Failed to open image sharing"
    open val imageLoadFailed: String = "Image loading failed"

    // Group
    open val groupMembers: String = "Members"
    open val groupOnlineMembers: String = "Online"
    open val groupJoin: String = "Join Group"
    open val groupLeave: String = "Leave Group"
    open val groupRepresentation: String = "Group Representation"
    open val groupRepresentationEnabled: String = "Display group"
    open val groupRepresentationDisabled: String = "Display group"
    open val groupRepresentationUpdating: String = "Updating representation..."
    open val groupRepresentationUpdateFailed: String = "Failed to update group representation"
    open val groupRepresentationSessionChanged: String = "Your account session changed. Try again."
    open val groupNotifications: String = "Group Notifications"
    open val groupNotificationsEnabled: String = "Group Notifications"
    open val groupNotificationsDisabled: String = "Group Notifications"
    open val groupNotificationsUpdating: String = "Updating notification preference..."
    open val groupNotificationsUpdateFailed: String = "Failed to update group notifications"
    open val groupNotificationsSessionChanged: String = "Your account session changed. Try again."
    open val groupRequested: String = "Requested"
    open val groupInviteOnly: String = "Invite Only"
    open val groupClosed: String = "Closed"
    open val groupRequestToJoin: String = "Request to Join"
    open val groupTabDetails: String = "Details"
    open val groupTabPosts: String = "Posts"
    open val groupNoPosts: String = "No posts"
    open val loading: String = "Loading..."
    open val groupInstances: String = "Group Instances"
    open val groupInstancesEmpty: String = "No group rooms are open right now"
    open val groupInstancesLoadFailed: String = "Couldn't load group rooms"
    open val groupTabMembers: String = "Members"
    open val groupTabGallery: String = "Gallery"
    open val groupOwner: String = "Owner"
    open val groupJoinedAt: String = "Joined At"
    open val groupCreatedAt: String = "Created At"
    open val groupUpdatedAt: String = "Updated At"
    open val groupMemberSync: String = "Member Sync"
    open val groupLastPost: String = "Last Post"
    open val groupPrivacy: String = "Privacy"
    open val groupJoinState: String = "Join State"
    open val groupMembershipStatus: String = "Membership Status"
    open val groupVerifiedLabel: String = "Verified"
    open val groupSearchableLabel: String = "Searchable"
    open val groupAllowJoinPrompt: String = "Allow Join Prompt"
    open val groupAgeVerificationCode: String = "Age Verification Code"
    open val groupAgeVerificationSlots: String = "Age Verification Slots"
    open val groupAgeVerificationAvailable: String = "Age Verification Available"
    open val groupTransferTarget: String = "Transfer Target"
    open val groupDescription: String = "Group Description"
    open val groupRules: String = "Group Rules"
    open val groupTags: String = "Tags"
    open val groupBadges: String = "Badges"
    open val groupLanguages: String = "Languages"
    open val groupLinks: String = "Links"
    open val groupMembersTitle: String = "Members"

    // User Note
    open val userNoteEditTitle: String = "Edit Note"
    open val userNoteSaved: String = "Note saved"

    // Local profile actions
    open val userMutualGroups: String = "Mutual Groups"
    open val inventoryCreditsTitle: String = "VRChat Credits"
    open val inventoryCreditsUnavailable: String = "Credits are unavailable for this account"
    open val inventoryCreditsLoadFailed: String = "Couldn't load your VRChat Credits"
    open val profileBoop: String = "Boop"
    open val profileBoopSuccess: String = "Boop sent"
    open val profileBoopAlreadySent: String = "Already booped"
    open val profileBoopDisabled: String = "This user has turned off Boops"
    open val profileBlock: String = "Block user"
    open val profileUnblock: String = "Unblock user"
    open val profileBlockStatusChecking: String = "Checking block status..."
    open val profileBlockStatusRetry: String = "Retry block status"
    open val profileBlockStatusUnavailable: String = "Block status is unavailable"
    open val profileBlockConfirmTitle: String = "Block this user?"
    open val profileBlockConfirmMessage: String = "Block %name%?"
    open val profileUnblockConfirmTitle: String = "Unblock this user?"
    open val profileUnblockConfirmMessage: String = "Unblock %name%?"
    open val profileBlockSuccess: String = "User blocked"
    open val profileUnblockSuccess: String = "User unblocked"
    open val profileBlockFailed: String = "Couldn't block this user. Try again."
    open val profileUnblockFailed: String = "Couldn't unblock this user. Try again."
    open val profileBlockStatusLoadFailed: String = "Couldn't load block status. Try again."
    open val boopSelectorTitle: String = "Boop %name%"
    open val boopSend: String = "Send"
    open val boopEmojiDefault: String = "Default"
    open val boopEmojiHeart: String = "Heart"
    open val boopEmojiWave: String = "Wave"
    open val boopEmojiLaugh: String = "Laugh"
    open val boopEmojiLike: String = "Like"
    open val boopEmojiThink: String = "Think"
    open val boopEmojiSurprise: String = "Surprise"
    open val boopEmojiAngry: String = "Angry"
    open val profileInviteToMyInstance: String = "Invite to My Instance"
    open val profileInviteSent: String = "Invite sent"
    open val profileInviteNotInInstance: String = "You are not in an instance"
    open val profileImageInvite: String = "Send Image Invite"
    open val imageInviteTitle: String = "Image invite to %name%"
    open val imageInvitePreparing: String = "Preparing the Gallery image..."
    open val imageInviteReady: String = "This Gallery image is ready to send."
    open val imageInviteSending: String = "Sending image invite..."
    open val imageInvitePrepareFailed: String = "The Gallery image could not be prepared."
    open val imageInviteSendFailed: String = "The image invite could not be sent."
    open val imageInviteSessionChanged: String = "Your account session changed. Choose the image again."
    open val imageInviteChooseAnother: String = "Choose Another"
    open val imageInviteSend: String = "Send"
    open val imageInviteSent: String = "Image invite sent"
    open val profileRequestInvite: String = "Request Invite"
    open val profileRequestInviteSent: String = "Invite request sent"
    open val inviteMessageSelectorInviteTitle: String = "Invite %name%"
    open val inviteMessageSelectorRequestTitle: String = "Request an invite from %name%"
    open val inviteMessageSelectorLoadFailed: String = "Couldn't load invite messages"
    open val inviteMessageSelectorEmpty: String = "No invite messages are available"
    open val inviteMessageSelectorSend: String = "Send"
    open val inviteMessageSelectorSlot: String = "Slot %slot%"
    open val friendActivityTitle: String = "Activity"
    open val friendActivityRecentTogether: String = "Recently together"
    open val friendActivityLastTogether: String = "Last met"
    open val friendActivityLastActivity: String = "Last activity"
    open val friendActivityMeetingCount: String = "Meetups"
    open val friendActivityTogetherTime: String = "Time together"
    open val friendActivityObservedHint: String = "Observed while VRCM was running"
    open val friendActivityTimelineTitle: String = "Friend activity"
    open val friendActivityTimelineEmpty: String = "No observed activity yet"
    open val friendActivityTimelineError: String = "Friend activity could not be loaded"
    open val friendActivityLoadMoreError: String = "More activity could not be loaded"
    open val friendActivityFilterAll: String = "All"
    open val friendActivityFilterPresence: String = "Online & offline"
    open val friendActivityFilterLocation: String = "Location"
    open val friendActivityFilterProfile: String = "Profile"
    open val friendActivityFilterMeetup: String = "Meetups"
    open val friendActivityBack: String = "Back"
    open val friendActivityShowTimeline: String = "Recent activity"
    open val friendActivityHideTimeline: String = "Hide activity"
    open val friendActivityDurationHours: String = "%hours% h %minutes% min"
    open val friendActivityDurationMinutes: String = "%minutes% min"
    open val friendActivityEventOnline: String = "Came online"
    open val friendActivityEventOffline: String = "Went offline"
    open val friendActivityEventLocation: String = "Changed world"
    open val friendActivityEventStatus: String = "Updated status"
    open val friendActivityEventBio: String = "Updated bio"
    open val friendActivityBioDiffShow: String = "Show latest change"
    open val friendActivityBioDiffHide: String = "Back to bio"
    open val friendActivityEventMet: String = "Joined the same instance"
    open val friendActivityEventLeft: String = "Left the shared instance"
    open val friendActivityAccessPublic: String = "Public"
    open val friendActivityAccessFriendsPlus: String = "Friends+"
    open val friendActivityAccessFriends: String = "Friends"
    open val friendActivityAccessInvitePlus: String = "Invite+"
    open val friendActivityAccessInvite: String = "Invite"
    open val friendActivityAccessGroup: String = "Group"
    open val friendActivityAccessUnknown: String = "Unknown"
    open val recentWorldsTitle: String = "Recent Worlds"
    open val recentWorldsEmpty: String = "No recent worlds"
    open val inventoryTitle: String = "Inventory"
    open val inventoryEmpty: String = "No inventory items"
    open val inventorySessionMissing: String = "Sign in to view inventory"
    open val inventoryLoadFailed: String = "Inventory could not be loaded"
    open val inventoryRefreshFailed: String = "Inventory could not be refreshed"
    open val inventoryLoadMoreFailed: String = "More inventory items could not be loaded"
    open val inventoryFilterType: String = "Type"
    open val inventoryFilterArchived: String = "Archive"
    open val inventoryFilterAll: String = "All"
    open val inventoryFilterActive: String = "Current"
    open val inventoryFilterArchivedOnly: String = "Archived"
    open val inventorySortLabel: String = "Sort"
    open val inventorySortNewestUpdated: String = "Recently updated"
    open val inventorySortNewestCreated: String = "Recently acquired"
    open val inventorySortOldestUpdated: String = "Least recently updated"
    open val inventorySortOldestCreated: String = "First acquired"
    open val inventoryTypeBundle: String = "Bundle"
    open val inventoryTypeDroneSkin: String = "Drone skin"
    open val inventoryTypeEmoji: String = "Emoji"
    open val inventoryTypePortalSkin: String = "Portal skin"
    open val inventoryTypeProp: String = "Prop"
    open val inventoryTypeSticker: String = "Sticker"
    open val inventoryTypeWarpEffect: String = "Warp effect"
    open val inventoryCount: String = "%d items"
    open val inventoryUnknownName: String = "Unnamed item"
    open val inventoryExpires: String = "Expires: %s"
    open val inventoryQuantifiable: String = "Quantity tracked"
    open val inventoryArchivedBadge: String = "Archived"
    open val retry: String = "Retry"
    open val avatars: String = "Avatars"
    open val friendListPagerAllAvatars: String = "All Avatars"
    open val favoritesTitle: String = "Favorites"
    open val favoritesPlayers: String = "Players"
    open val favoritesFriends: String = "Friends"
    open val myGroups: String = "My Groups"
    open val myGroupsEmpty: String = "You haven't joined any groups yet"
    open val myGroupsLoadFailed: String = "Couldn't load your groups"
    open val favoritesEmpty: String = "Nothing here yet"
    open val favoritesLoadFailed: String = "Couldn't load this collection"
    open val favoritesGroupLoadFailed: String = "Couldn't load favorite groups"
    open val favoritesCacheReadFailed: String = "Couldn't read favorites cache"
    open val favoriteWorldsLoadFailed: String = "Couldn't load favorite worlds"
    open val favoriteAvatarsLoadFailed: String = "Couldn't load favorite avatars"
    open val favoriteWorldCacheSaveFailed: String = "Couldn't save favorite worlds cache"
    open val favoriteAvatarCacheSaveFailed: String = "Couldn't save favorite avatars cache"
    open val favoriteGroupClearAction: String = "Clear group"
    open val favoriteGroupClearTitle: String = "Clear \"%s\"?"
    open val favoriteGroupClearMessage: String =
        "Remove %d favorite relationships from this group? The group itself will remain. This cannot be undone."
    open val favoriteGroupClearing: String = "Clearing..."
    open val favoriteGroupClearSuccess: String = "Favorite group cleared"
    open val favoriteGroupClearFailed: String = "Couldn't clear favorite group"
    open val favoriteGroupClearSyncFailed: String =
        "The group was cleared, but local data couldn't be refreshed"
    open val favoriteSelectionAction: String = "Select favorites to remove"
    open val favoriteSelectionSelectedCount: String = "%d selected"
    open val favoriteSelectionSelectAll: String = "Select all shown"
    open val favoriteSelectionClearSelection: String = "Clear shown"
    open val favoriteSelectionRemoveSelected: String = "Remove selected favorites"
    open val favoriteSelectionRemoveConfirmTitle: String = "Remove favorites?"
    open val favoriteSelectionRemoveConfirmMessage: String =
        "Remove the selected %d favorites? This cannot be undone."
    open val favoriteSelectionRemovingProgress: String = "Removing %d of %d"
    open val favoriteSelectionRemoveSuccess: String = "Removed %d favorites"
    open val favoriteSelectionRemovePartialFailure: String = "Removed %d favorites; %d failed"
    open val favoriteSelectionRemoveFailed: String = "Failed to remove %d favorites"
    open val groupSelectionAction: String = "Select groups to leave"
    open val groupSelectionLeaveSelected: String = "Leave selected groups"
    open val groupSelectionLeaveConfirmTitle: String = "Leave groups?"
    open val groupSelectionLeaveConfirmMessage: String = "Leave the selected %d groups?"
    open val groupSelectionLeavingProgress: String = "Leaving %d of %d groups"
    open val groupSelectionLeaveSuccess: String = "Left %d groups"
    open val groupSelectionLeavePartialFailure: String = "Left %d groups; %d failed"
    open val groupSelectionLeaveFailed: String = "Failed to leave %d groups"
    open val personalDrawerTitle: String = "Personal navigation"
    open val mainNavigationHome: String = "Home"
    open val mainNavigationSearch: String = "Search"
    open val mainNavigationNotifications: String = "Inbox"
    open val mainNavigationFriends: String = "Friends"
    open val homeTabLocation: String = "Locations"
    open val homeLocationSourceFriends: String = "Friend Locations"
    open val homeTabActivity: String = "Activity"
    open val drawerMyProfile: String = "My profile"
    open val drawerSettings: String = "Settings"
    open val logoutConfirmTitle: String = "Log out?"
    open val logoutConfirmMessage: String = "You will need to sign in again to use VRCM."
    open val back: String = "Back"
    open val refresh: String = "Refresh"
    open val close: String = "Close"

    // User Created Worlds & Avatars
    open val userCreatedWorlds: String = "Created Worlds"
    open val userCreatedAvatars: String = "Created Avatars"
    open val userCreatedWorldsCount: String = "Worlds: %d"
    open val userCreatedAvatarsCount: String = "Avatars: %d"
    open val userCreatedLoading: String = "Loading..."
    open val userCreatedEmpty: String = "No items"
    open val userFavoritedWorlds: String = "Favorited Worlds"
    open val userFavoritedAvatars: String = "Favorited Avatars"

    // Avatar Profile
    open val avatarProfileVersion: String = "Version"
    open val avatarProfileStatus: String = "Status"
    open val avatarProfileCreated: String = "Created"
    open val avatarProfileUpdated: String = "Updated"
    open val avatarProfilePlatforms: String = "Platforms"
    open val avatarProfileBanned: String = "This avatar has been banned."
    open val avatarProfileSwitched: String = "Switched to this avatar."
    open val avatarProfileCopied: String = "Copied and switched to this avatar."
    open val avatarProfileSelectFailed: String = "Failed to apply this avatar. Please try again later."
    open val avatarProfileMoreActions: String = "More actions"
    open val avatarProfileActionChecking: String = "Checking avatar availability..."
    open val avatarProfileActionCurrent: String = "Currently using this avatar"
    open val avatarProfileActionSwitch: String = "Switch avatar"
    open val avatarProfileActionNotCopyable: String = "This avatar cannot be copied"
    open val avatarProfileActionCheckFailed: String = "Unable to verify avatar availability"
    open val avatarModerationChecking: String = "Checking avatar block status..."
    open val avatarModerationBlock: String = "Block this avatar"
    open val avatarModerationUnblock: String = "Unblock this avatar"
    open val avatarModerationRetry: String = "Retry block status check"
    open val avatarModerationUnavailable: String = "Avatar blocking is unavailable"
    open val avatarModerationBlocking: String = "Blocking..."
    open val avatarModerationUnblocking: String = "Unblocking..."
    open val avatarModerationBlockConfirmTitle: String = "Block this avatar?"
    open val avatarModerationBlockConfirmMessage: String =
        "Block this avatar across VRChat? You can unblock it later."
    open val avatarModerationUnblockConfirmTitle: String = "Unblock this avatar?"
    open val avatarModerationUnblockConfirmMessage: String =
        "Remove the global block for this avatar?"
    open val avatarModerationBlocked: String = "This avatar has been blocked."
    open val avatarModerationUnblocked: String = "This avatar has been unblocked."
    open val avatarModerationLoadFailed: String = "Unable to load the avatar block status."
    open val avatarModerationChangeFailed: String = "Failed to change the avatar block status."
    open val avatarProfileFallbackSelected: String = "Fallback avatar updated."
    open val avatarProfileFallbackIneligible: String =
        "This avatar is not eligible to be a fallback avatar."
    open val avatarProfileFallbackNotFound: String = "This avatar is no longer available."
    open val avatarProfileFallbackUnauthorized: String =
        "Your session expired. Sign in again and retry."
    open val avatarProfileFallbackSelectFailed: String = "Failed to set the fallback avatar."
    open val avatarProfileFallbackActionSet: String = "Set as fallback avatar"
    open val avatarProfileFallbackActionCurrent: String = "Current fallback avatar"
    open val avatarProfileFallbackActionIneligible: String = "Unavailable as fallback avatar"
    open val avatarProfileFallbackActionSetting: String = "Setting fallback avatar..."
    open val avatarDeleteAction: String = "Delete avatar"
    open val avatarDeleteTitle: String = "Delete avatar?"
    open val avatarDeleteMessage: String =
        "Deleting \"%s\" will hide the avatar and permanently delete its associated files. This cannot be undone."
    open val avatarDeleteConfirm: String = "Delete"
    open val avatarDeleteDeleting: String = "Deleting..."
    open val avatarDeleteSuccess: String = "Avatar deleted."
    open val avatarDeleteBadRequest: String = "The delete request was rejected. Check the avatar and try again."
    open val avatarDeleteUnauthorized: String = "Your session is no longer authorized. Sign in again and retry."
    open val avatarDeleteForbidden: String = "You do not have permission to delete this avatar."
    open val avatarDeleteNotFound: String = "This avatar no longer exists."
    open val avatarDeleteInvalidResponse: String =
        "The server did not confirm the deletion. Refresh the avatar before trying again."
    open val avatarDeleteUnexpected: String = "Failed to delete the avatar. Please try again."
    open val avatarGalleryTitle: String = "Avatar Gallery"
    open val avatarGalleryEmpty: String = "No gallery images"
    open val avatarGalleryLoadFailed: String = "Failed to load avatar gallery"
    open val avatarGalleryLoadMoreFailed: String = "Failed to load more gallery images"
    open val avatarGalleryLoadMore: String = "Load more"
    open val avatarEditTitle: String = "Edit avatar"
    open val avatarEditName: String = "Name"
    open val avatarEditDescription: String = "Description"
    open val avatarEditContentTags: String = "Content tags"
    open val avatarEditContentHorror: String = "Horror"
    open val avatarEditContentGore: String = "Gore"
    open val avatarEditContentViolence: String = "Violence"
    open val avatarEditContentAdult: String = "Adult"
    open val avatarEditContentSex: String = "Sexual content"
    open val avatarEditStyles: String = "Styles"
    open val avatarEditStylesLoading: String = "Loading allowed styles..."
    open val avatarEditStylesEmpty: String = "VRChat did not return any available styles."
    open val avatarEditStylesLoadFailed: String = "Failed to load the allowed styles."
    open val avatarEditPrimaryStyle: String = "Primary style"
    open val avatarEditSecondaryStyle: String = "Secondary style"
    open val avatarEditNoStyle: String = "None"
    open val avatarEditAuthorTags: String = "Author tags"
    open val avatarEditAuthorTagsHint: String =
        "Separate tags with commas or new lines. The author_tag_ prefix is added automatically."
    open val avatarEditCover: String = "Cover image"
    open val avatarEditCoverHint: String =
        "JPEG, PNG, or WebP, up to 10 MiB; exported at 1920 x 1080"
    open val avatarEditChooseCover: String = "Choose cover image"
    open val avatarEditSaveMetadata: String = "Save avatar details"
    open val avatarEditUploadCover: String = "Update cover"
    open val avatarEditSavingMetadata: String = "Saving..."
    open val avatarEditUploadingCover: String = "Uploading..."
    open val avatarEditInvalidName: String = "Avatar name cannot be empty."
    open val avatarEditInvalidContentTags: String = "One or more content tags are not allowed."
    open val avatarEditInvalidStyle: String =
        "The selected style is no longer allowed. Reload the style list and try again."
    open val avatarEditNoChanges: String = "Avatar details have not changed."
    open val avatarEditMetadataSaved: String = "Avatar details updated."
    open val avatarEditMetadataSaveFailed: String = "Failed to update avatar details."
    open val avatarEditCoverSaved: String = "Avatar cover updated."
    open val avatarEditPublication: String = "Visibility"
    open val avatarEditPublicationPrivate: String = "Private"
    open val avatarEditPublicationPublic: String = "Public"
    open val avatarEditUpdatingPublication: String = "Updating visibility..."
    open val avatarEditPublishConfirmTitle: String = "Make this avatar public?"
    open val avatarEditPublishConfirmMessage: String =
        "Anyone will be able to copy and use this avatar. Only continue if you intend to share it publicly."
    open val avatarEditPublishConfirmAction: String = "Make public"
    open val avatarEditPublicationMadePublic: String = "Avatar is now public."
    open val avatarEditPublicationMadePrivate: String = "Avatar is now private."
    open val avatarEditPublicationBadRequest: String =
        "The visibility change was rejected. Check the avatar details and try again."
    open val avatarEditPublicationUnauthorized: String =
        "Your session is no longer authorized. Sign in again and retry."
    open val avatarEditPublicationForbidden: String =
        "You do not have permission to change this avatar's visibility."
    open val avatarEditPublicationNotFound: String =
        "This avatar could not be found. Refresh the page and try again."
    open val avatarEditPublicationFailed: String =
        "Failed to update avatar visibility. Please try again later."
    open val avatarEditCoverUploadFailed: String = "Failed to upload the cover image."
    open val avatarEditCoverAssignmentFailed: String =
        "The image was uploaded, but VRChat did not apply it to the avatar. Retry the cover update."
    open val avatarEditUnsupportedFormat: String =
        "Unsupported or mismatched image format. Choose a valid JPEG, PNG, or WebP file."
    open val avatarEditFileTooLarge: String = "The selected cover exceeds 10 MiB."
    open val avatarEditReadFailed: String = "Failed to read the selected cover image."
    open val avatarGalleryHint: String =
        "Upload an image to this avatar's Gallery; JPEG, PNG, WebP, or HEIC up to 50 MiB."
    open val avatarGalleryChooseImage: String = "Choose Gallery image"
    open val avatarGalleryUpload: String = "Upload to Gallery"
    open val avatarGalleryUploading: String = "Uploading to Avatar Gallery..."
    open val avatarGalleryRefreshing: String = "Refreshing Avatar Gallery..."
    open val avatarGalleryUploadFailed: String = "Failed to upload the Avatar Gallery image."
    open val avatarGalleryFileTooLarge: String = "The selected Gallery image exceeds 50 MiB."
    open val avatarGalleryReadFailed: String = "Failed to read the selected Gallery image."
    open val avatarGalleryRefreshFailed: String =
        "The image was uploaded, but Avatar Gallery could not be refreshed. Retry to refresh."
    open val avatarGallerySessionChanged: String =
        "The account changed while uploading. Select the image again."
    open val avatarGalleryPermissionDenied: String =
        "You can only upload Gallery images to your own avatar."
    open val avatarGalleryUploaded: String = "Avatar Gallery updated."
    open val avatarImpostorTitle: String = "Impostor"
    open val avatarImpostorAvailable: String = "This avatar has a generated Impostor."
    open val avatarImpostorEmpty: String = "This avatar has no generated Impostor."
    open val avatarImpostorDeleteAction: String = "Delete Impostor"
    open val avatarImpostorDeleting: String = "Deleting Impostor..."
    open val avatarImpostorVerifying: String = "Checking server status..."
    open val avatarImpostorDeleteConfirmationTitle: String = "Delete this Impostor?"
    open val avatarImpostorDeleteConfirmationMessage: String =
        "Delete the generated Impostor for \"%name%\"? This cannot be undone. " +
            "The avatar and its Unity packages will not be deleted."
    open val avatarImpostorDeleteSuccess: String = "Impostor deleted."
    open val avatarImpostorDeleteFailed: String =
        "VRChat still reports this Impostor, or the deletion failed. Try again."
    open val avatarImpostorVerificationFailed: String =
        "The deletion was accepted, but its server status could not be checked."
    open val avatarImpostorRetryVerification: String = "Check Impostor status again"
    open val avatarImpostorUnavailable: String = "No Impostor generated"
    open val avatarImpostorTaskStatus: String = "Generation task"
    open val avatarImpostorTaskEmpty: String = "No task submitted in this session"
    open val avatarImpostorCreate: String = "Create Impostor"
    open val avatarImpostorRebuild: String = "Rebuild Impostor"
    open val avatarImpostorSubmitting: String = "Submitting..."
    open val avatarImpostorStatusQueued: String = "Queued"
    open val avatarImpostorStatusProcessing: String = "Processing"
    open val avatarImpostorStatusCompleted: String = "Completed"
    open val avatarImpostorStatusFailed: String = "Failed"
    open val avatarImpostorStatusUnknown: String = "Server status: %s"
    open val avatarImpostorQueueEstimateLoading: String = "Loading queue estimate..."
    open val avatarImpostorQueueEstimateMinutes: String = "Estimated wait: about %minutes% min"
    open val avatarImpostorQueueEstimateUnavailable: String = "Queue estimate unavailable"
    open val avatarImpostorAuthenticationFailed: String = "Your session expired. Sign in again and retry."
    open val avatarImpostorPermissionFailed: String = "You do not have permission to generate this Impostor."
    open val avatarImpostorNotFound: String = "This avatar is no longer available."
    open val avatarImpostorConflict: String = "An Impostor task is already active for this avatar."
    open val avatarImpostorRateLimited: String = "Too many requests. Retry later."
    open val avatarImpostorServerFailed: String = "VRChat could not start the Impostor task. Retry later."
    open val avatarImpostorUnknownFailed: String = "Failed to start the Impostor task."
    open val hiddenModel: String = "Hidden model"
    open val hiddenModelCannotView: String = "Cannot view hidden model info"
    open val hiddenWorld: String = "Hidden world"
    open val hiddenWorldCannotView: String = "Cannot view hidden world info"
    open val meetupCardTitle: String = "Nameplate"
    open val meetupCardEdit: String = "Edit"
    open val meetupCardRotatePreview: String = "Landscape preview"
    open val meetupCardSaveImage: String = "Save to gallery"
    open val meetupCardDone: String = "Done"
    open val meetupCardPhoto: String = "Photo"
    open val meetupCardLayout: String = "Layout"
    open val meetupCardContent: String = "Content"
    open val meetupCardStyle: String = "Style"
    open val meetupCardProfileBackground: String = "Profile background"
    open val meetupCardAlbum: String = "Photo library"
    open val meetupCardGallery: String = "VRChat Gallery"
    open val meetupCardPickPhotoTitle: String = "Choose a photo"
    open val meetupCardInfoBar: String = "Info bar"
    open val meetupCardSpotlight: String = "Spotlight"
    open val meetupCardSideTag: String = "Side tag"
    open val meetupCardShowAvatar: String = "Avatar"
    open val meetupCardShowPronouns: String = "Pronouns"
    open val meetupCardShowLanguages: String = "Languages"
    open val meetupCardShowStatus: String = "Status"
    open val meetupCardShowStatusDescription: String = "Status message"
    open val meetupCardShowGroupBanner: String = "Represented group"
    open val meetupCardGroupDisplayStyle: String = "Group display"
    open val meetupCardGroupBanner: String = "Cover"
    open val meetupCardGroupIconName: String = "Icon and name"
    open val meetupCardShortText: String = "Short message"
    open val meetupCardShowQrCode: String = "Profile QR code"
    open val meetupCardIconFrame: String = "Icon frame"
    open val meetupCardProfileEffect: String = "Profile effect"
    open val meetupCardNameplateEffect: String = "Nameplate effect"
    open val meetupCardPortrait: String = "Portrait"
    open val meetupCardLandscape: String = "Landscape"
    open val meetupCardPhotoTarget: String = "Apply to"
    open val meetupCardPhotoTargetBoth: String = "Both orientations"
    open val meetupCardAccentColor: String = "Theme color"
    open val meetupCardScrim: String = "Overlay strength"
    open val meetupCardCropTitle: String = "Adjust crop"
    open val meetupCardShortTextTooLong: String = "Keep the message within 80 characters."
    open val meetupCardQrDescription: String = "VRChat profile QR code"
    open val meetupCardQrLinkType: String = "QR code link"
    open val meetupCardQrLinkVrchat: String = "VRChat web (universal)"
    open val meetupCardQrProfileLinks: String = "Profile links"
    open val meetupCardQrLimit: String = "Up to %d QR codes at a time"
    open val meetupCardQrLabelVrchat: String = "VRChat"
    open val copyOfficialUrl: String = "Copy official URL"
    open val officialUrlCopied: String = "Link copied"
    open val officialUrlCopyFailed: String = "Could not copy the official URL"
    open val shareOfficialUrl: String = "Share official URL"
    open val officialUrlShareFailed: String = "Could not open the share sheet"
    open val officialLinkPromptTitle: String = "Open VRChat content"
    open val officialLinkPromptMessage: String =
        "A VRChat %s link or ID was found in your clipboard. Open it in VRCM?"
    open val officialLinkOpen: String = "Open"
    open val officialLinkOpenFailed: String =
        "Could not open this content. It may have expired, been removed, or be unavailable to this account."
    open val officialLinkTypeUser: String = "user"
    open val officialLinkTypeWorld: String = "world"
    open val officialLinkTypeGroup: String = "group"
    open val officialLinkTypeAvatar: String = "avatar"
    open val meetupCardLayoutPerOrientation: String =
        "Portrait and landscape keep separate layouts."
    open val meetupCardRefreshPartial: String = "Some card content could not be refreshed."
    open val meetupCardRefreshFailed: String = "The card could not be refreshed. Showing saved content."
    open val meetupCardPhotoFailed: String = "The selected photo could not be used."
    open val meetupCardSaveFailed: String = "The change could not be saved."
    open val meetupCardSessionExpired: String = "The photo selection expired. Choose the photo again."

    // Player moderation cleanup
    open val playerModerationCleanupTitle: String = "Player moderation cleanup"
    open val playerModerationCleanupDescription: String =
        "Choose one server-backed override type to remove for this account. This cannot be undone."
    open val playerModerationCleanupSelectType: String = "Override type"
    open val playerModerationCleanupTargetCount: String = "%count% targets"
    open val playerModerationCleanupAction: String = "Clear selected type"
    open val playerModerationCleanupConfirmTitle: String = "Clear this override type?"
    open val playerModerationCleanupConfirmMessage: String =
        "Clear %type% records for %count% targets? This action cannot be undone."
    open val playerModerationCleanupPreparing: String = "Checking current records..."
    open val playerModerationCleanupProgress: String = "Clearing %processed% of %total% targets..."
    open val playerModerationCleanupSuccess: String = "Cleared records for %removed% targets."
    open val playerModerationCleanupNoRecords: String = "No matching records remain."
    open val playerModerationCleanupPartialFailure: String =
        "Cleared %removed% targets; %failed% could not be cleared."
    open val playerModerationCleanupFailure: String =
        "Could not confirm that all selected records were cleared. Refresh to check the current state."
}

val strings: LocaleStrings
    @Composable
    get() {
        return when (LocalSettingsState.current.value.languageTag) {
            LanguageTag.EN -> LocaleStringsEn
            LanguageTag.JA -> LocaleStringsJa
            LanguageTag.ZH_HANS -> LocaleStringsZhHans
            LanguageTag.ZH_HANT -> LocaleStringsZhHant
            else -> LocaleStringsEn
        }
    }
