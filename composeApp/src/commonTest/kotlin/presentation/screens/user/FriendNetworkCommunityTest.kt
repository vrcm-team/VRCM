...
class FriendNetworkCommunityTest {
    ...
    private fun clique(prefix: String, size: Int): Pair<List<MutualFriendData>, List<Pair<String, String>>> {
        val members = (1..size).map { node("$prefix$it") }
        val pairs = mutableListOf<Pair<String, String>>()
        for (i in 1..size) for (j in i + 1..size) pairs.add("$prefix$i" to "$prefix$j")
        return members to pairs
    }

    @Test
    fun anchoredIdsSurviveSizeFlip() {
        // 上一轮：a 圈(5人)=0，b 圈(4人)=1；本轮 b 圈长到 6 人反超
        val (aNodes, aPairs) = clique("a", 5)
        val (bNodes, bPairs) = clique("b", 6)
        val graph = symmetricEdges(*(aPairs + bPairs).toTypedArray())
        val previous = buildMap {
            (1..5).forEach { put("a$it", 0) }
            (1..4).forEach { put("b$it", 1) }
        }
        val communities = FriendNetworkScreenModel.assignCommunities(
            aNodes + bNodes, graph, previousAssignments = previous
        )
        // 人数排名翻转，但编号（颜色）不洗牌
        assertEquals(0, communities["a1"])
        assertEquals(1, communities["b1"])
    }

    @Test
    fun lowOverlapCommunityGetsFreshIdAfterCooldown() {
        // 上一轮：x 圈=0；本轮的圈与 x 圈只有 1 人重叠（< max(2, 30%)）→ 判新圈，
        // 且编号 0 处于冷却期不得复用 → 分配 1
        val members = listOf(node("x1")) + (1..8).map { node("y$it") }
        val pairs = mutableListOf<Pair<String, String>>()
        val ids = members.map { it.id }
        for (i in ids.indices) for (j in i + 1 until ids.size) pairs.add(ids[i] to ids[j])
        val graph = symmetricEdges(*pairs.toTypedArray())
        val previous = (1..5).associate { "x$it" to 0 }

        val communities = FriendNetworkScreenModel.assignCommunities(
            members, graph, previousAssignments = previous
        )
        assertEquals(1, communities["y1"])
    }

    @Test
    fun splitLargerHalfInheritsIdAndSmallerGetsFresh() {
        // 上一轮：p1..p8 同为 0，q 圈=1；本轮 p 圈裂成 5 人和 3 人两团
        val (p1Nodes, p1Pairs) = clique("m", 5)
        val (p2Nodes, p2Pairs) = clique("n", 3)
        val (qNodes, qPairs) = clique("q", 4)
        val graph = symmetricEdges(*(p1Pairs + p2Pairs + qPairs).toTypedArray())
        val previous = buildMap {
            (1..5).forEach { put("m$it", 0) }
            (1..3).forEach { put("n$it", 0) }
            (1..4).forEach { put("q$it", 1) }
        }
        val communities = FriendNetworkScreenModel.assignCommunities(
            p1Nodes + p2Nodes + qNodes, graph, previousAssignments = previous
        )
        // 大的一半继承 0，小的一半拿全新编号 2（0、1 都被占/冷却），q 圈保持 1
        assertEquals(0, communities["m1"])
        assertEquals(1, communities["q1"])
        assertEquals(2, communities["n1"])
    }
